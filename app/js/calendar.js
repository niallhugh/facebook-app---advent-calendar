...
PINT.calendarInfo = [];
if ( !$.cookie('ROCSTUDIOcalendar') ) {
	$.cookie('ROCSTUDIOcalendar', '', { path:'/', expires:60 });
} 
else {
	PINT.calendarInfo = $.cookie('ROCSTUDIOcalendar').split(',');
}
...
(function ($) {
    ...
    $.fn.PINT_calendar = function(options) {
         ...
         /** Get todays date **/
          /** Add today variable to the settings object in place of date object value **/
        var today = new Date().getDate();
        
        /** USER ADDED SETTINGS ( Pass into function/method call init ) **/
        var settings = $.extend( {
            // Defaults
            date:today,  //25, //new Date().getDate();  Set Limit Day ( all days before this selectable )
        ...
        return this.each(function() {
            ...
            function PINT_calendarActivateDay(aObj, bRaiseOverlay) {
                ...
                if ($.inArray(sDay, PINT.calendarInfo) == -1) {
                    ...
                    $.cookie('ROCSTUDIOcalendar', PINT.calendarInfo.join(','), { path : '/', expires : 60 });
                }
                $(aObj)
                    .stop()
    ...
    $.fn.PINT_showOverlay = function(content) {
        this.each(function() {
            if( !$('#overlay-main').length ) {
                $('body').prepend($('<div id="overlay-main"><div id="overlay-body"><div id="overlay-close"></div></div></div>').css({
                    display: 'none'
                }));
            }
            ...
            var closeCode = '<div id="overlay-close"></div>';    
